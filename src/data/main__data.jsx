
export const main = {
   groups: [
      {
         name: 'ac dc',
         image: '../pictures/groups/ACDC.jpeg',
      },
      {
         name: 'the beatles',
         image: '../pictures/groups/beatles.jpeg',
      },
      {
         name: 'boston',
         image: '../pictures/groups/boston.jpeg',
      },
      {
         name: 'deep purple',
         image: '../pictures/groups/deep_purple.jpeg',
      },
      {
         name: 'europe',
         image: '../pictures/groups/europe.jpeg',
      },
      {
         name: 'fleetwood mac',
         image: '../pictures/groups/fleetwood_mac.jpeg',
      },
      {
         name: 'green day',
         image: '../pictures/groups/green_day.jpeg',
      },
      {
         name: 'gund n\' roses',
         image: '../pictures/groups/guns_n_roses.jpeg',
      },
      {
         name: 'journey',
         image: '../pictures/groups/journey.jpeg',
      },
      {
         name: 'led zeppelin',
         image: '../pictures/groups/led_zeppelin.webp',
      },

      {
         name: 'megadeth',
         image: '../pictures/groups/megadeth.webp',
      },
      {
         name: 'metallica',
         image: '../pictures/groups/metallica.webp',
      },
      {
         name: 'nirvana',
         image: '../pictures/groups/nirvana.webp',
      },
      {
         name: 'pink floyd',
         image: '../pictures/groups/pink_floyd.jpeg',
      },
      {
         name: 'queen',
         image: '../pictures/groups/queen.jpeg',
      },
      {
         name: 'rainbow',
         image: '../pictures/groups/rainbow.webp',
      },
      {
         name: 'rammstein',
         image: '../pictures/groups/rammstein.jpeg',
      },
      {
         name: 'red hot chilli peppers',
         image: '../pictures/groups/red_hot_chili_peppers.webp',
      },
      {
         name: 'scorpions',
         image: '../pictures/groups/scorpions.webp',
      },
      {
         name: 'system of a down',
         image: '../pictures/groups/system_of_a_down.webp',
      },

   ],
   topAlbums: [
      {
         image: '../pictures/top_albums/guardians_of_the_galaxy.jpeg',
         name: 'guardians of the galaxy: Awesome Mix Vol. 1',
         year: '29.07.2014',
      },
      {
         image: '../pictures/top_albums/nevermind.webp',
         name: 'nevermind',
         year: '24.09.1991',
      },
      {
         image: '../pictures/top_albums/american_idiot.webp',
         name: 'american idiot',
         year: '21.09.2004',
      }
   ],
   topSongs: [
      {
         id: '4',
         place: 1,
         name: 'still loving you',
         album: 'scorpions',
         duration: '',
         src: '',
         image: '../pictures/top/love_at_first_sting.jpeg'
      },
      {
         id: '1',
         place: 2,
         name: 'wind of change',
         album: 'scorpions',
         duration: '',
         src: '',
         image: '../pictures/top/crazy_world.jpeg'
      },
      {
         id: '2',
         place: 3,
         name: 'november rain',
         album: 'guns n roses',
         duration: '',
         src: '',
         image: '../pictures/top/use_your_ilusion_1.jpeg',
      },
      {
         id: '3',
         place: 4,
         name: 'starway to heaven',
         album: 'led zeppelin',
         duration: '',
         src: '',
         image: '../pictures/top/led_zeppelin_4.jpeg'
      },
      {
         id: '4',
         place: 5,
         name: 'nothing else matters',
         album: 'metallica',
         duration: '',
         src: '',
         image: '../pictures/top/black-album.webp'
      },
      {
         id: '1',
         place: 6,
         name: 'send me an angel',
         album: 'scorpions',
         duration: '',
         src: '',
         image: '../pictures/top/crazy_world.jpeg'
      },
      {
         id: '0',
         place: 7,
         name: 'bohemian rhapsody',
         album: 'queen',
         duration: '',
         src: '',
         image: '../pictures/top/a_night_in_the_opera.jpeg'
      },
      {
         id: '10',
         place: 8,
         name: 'fade to black',
         album: 'metallica',
         duration: '',
         src: '',
         image: '../pictures/top/ride-the-lightning.png'
      },
      {
         id: '2',
         place: 9,
         name: 'sweat child of mine',
         album: 'guns n roses',
         duration: '',
         src: '',
         image: '../pictures/top/apetite_for_distruction.jpeg',
      },
      {
         id: '4',
         place: 10,
         name: 'always somewere',
         album: 'scorpions',
         duration: '',
         src: '',
         image: '../pictures/top/lovedrive.webp'
      }
   ],
   years: [
      {
         year: '1970',
         image: '../pictures/years/70_paranoid.jpeg',
         name: 'paranoid',
         group: 'black sabbath',
      },
      {
         year: '1971',
         image: '../pictures/years/71.jpeg',
         name: 'starway to heaven',
         group: 'led zeppelin',
      },
      {
         year: '1972',
         image: '../pictures/years/72.jpeg',
         name: 'smoke in the water',
         group: 'deep purple',
      },
      {
         year: '1973',
         image: '../pictures/years/73.jpeg',
         name: 'dream on',
         group: 'aerosmith',
      },
      {
         year: '1974',
         image: '../pictures/years/74.jpeg',
         name: 'sweat home alabama',
         group: 'cynyrd skynyrd',
      },
      {
         year: '1975',
         image: '../pictures/years/75.png',
         name: 'bohemian rhapsody',
         group: 'queen',
      },
      {
         year: '1976',
         image: '../pictures/years/76.jpeg',
         name: 'hotel california',
         group: 'eagles',
      },
      {
         year: '1977',
         image: '../pictures/years/77.jpeg',
         name: 'the chain',
         group: 'fleetwood mac',
      },
      {
         year: '1978',
         image: '../pictures/years/78.jpeg',
         name: 'runnin\' with the devil',
         group: 'van halen',
      },
      {
         year: '1979',
         image: '../pictures/years/79.jpeg',
         name: 'another one bites the dust',
         group: 'queen',
      },
      {
         year: '1980',
         image: '../pictures/years/80.jpeg',
         name: 'back in black',
         group: 'ac dc',
      },
      {
         year: '1981',
         image: '../pictures/years/81.webp',
         name: 'start me up',
         group: 'the rolling stones',
      },
      {
         year: '1982',
         image: '../pictures/years/82.jpeg',
         name: 'should i stay or should i go',
         group: 'the clash',
      },
      {
         year: '1983',
         image: '../pictures/years/83.jpeg',
         name: 'every breath you take',
         group: 'the police',
      },
      {
         year: '1984',
         image: '../pictures/years/84.jpeg',
         name: 'for whom the bell tools',
         group: 'metallica',
      },
      {
         year: '1985',
         image: '../pictures/years/85.jpeg',
         name: 'money for nothing',
         group: 'dire straits',
      },
      {
         year: '1986',
         image: '../pictures/years/86.jpeg',
         name: 'livin\' on a prayer',
         group: 'bon jovi',
      },
      {
         year: '1987',
         image: '../pictures/years/87.jpeg',
         name: 'sweat child o\' mine',
         group: 'guns n\' roses',
      },
      {
         year: '1988',
         image: '../pictures/years/88.jpeg',
         name: 'orange crush',
         group: 'R.E.M',
      },
      {
         year: '1989',
         image: '../pictures/years/89.jpeg',
         name: 'kickstar my heart',
         group: 'motley crue',
      },
      {
         year: '1990',
         image: '../pictures/years/90.jpeg',
         name: 'wind of change',
         group: 'scorpions',
      },
      {
         year: '1991',
         image: '../pictures/years/91.jpeg',
         name: 'smeels like teen spirit',
         group: 'nirvana',
      },
      {
         year: '1992',
         image: '../pictures/years/92.jpeg',
         name: 'creep',
         group: 'radiohead',
      },
      {
         year: '1993',
         image: '../pictures/years/93.jpeg',
         name: 'daugher',
         group: 'pearl jam',
      },
      {
         year: '1994',
         image: '../pictures/years/94.jpeg',
         name: 'black hole sun',
         group: 'soundgarden',
      },
      {
         year: '1995',
         image: '../pictures/years/95.jpeg',
         name: 'wonderwall',
         group: 'oasis',
      },
      {
         year: '1996',
         image: '../pictures/years/96.jpeg',
         name: '1979',
         group: 'smashing pumpkins',
      },
      {
         year: '1997',
         image: '../pictures/years/97.jpeg',
         name: 'semi-charmed life',
         group: ' third eye blind',
      },
      {
         year: '1998',
         image: '../pictures/years/98.jpeg',
         name: 'fly away',
         group: 'lenny kravetz',
      },
      {
         year: '1999',
         image: '../pictures/years/99.png',
         name: 'all the small things',
         group: 'blink-182',
      },
      {
         year: '2000',
         image: '../pictures/years/00.jpeg',
         name: 'in the end',
         group: 'linkin\' park',
      },
   ]
}